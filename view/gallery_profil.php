<?php
	...
	$count = $bdd->query("SELECT COUNT(*) as nb_pictures FROM pictures INNER JOIN menbre ON pictures.usersid = menbre.id WHERE menbre.id = {$_SESSION['id']}");
...
<section>
	<div id="option">
		<a id="style" href="indexView.php?page=option"><i class="fas fa-cogs"></i> Options</a>
	</div>
	<div id="gallery">
		<div class="box_gallery">
			<?php
			if (isset($_GET['supprimer']) && !empty($_GET['supprimer']) && isset($_GET['pictures_id']) && !empty($_GET['pictures_id']))
				{
					$secure_picture_id = htmlspecialchars($_GET['pictures_id']);
					$secure_supprimer = htmlspecialchars($_GET['supprimer']);

					if ($_GET['supprimer'] == 1)
					{
						$delete_picture = $bdd->prepare("DELETE FROM pictures WHERE pictures_id = ? AND usersid = ?");
						$delete_picture->execute(array($secure_picture_id, $_SESSION['id']));
					}
				}
				...
				$images = $bdd->query("SELECT * FROM pictures INNER JOIN menbre ON pictures.usersid = menbre.id WHERE menbre.id = {$_SESSION['id']} ORDER BY pictures_id DESC LIMIT " . (($current_page - 1) * $picture_page). ", $picture_page");
					...
					{	
						echo "
							<div id='com_like'>
								...
								<div class='size_awesome'><a href='indexView.php?like=1&page=gallery1&pictures_id={$donnees['pictures_id']}&nb_page={$_GET['nb_page']}'><i class='fas fa-thumbs-up'></i>(" . $nb_likes . ")</a></div>
								<div><a href='indexView.php?page=gallery1&pictures_id={$donnees['pictures_id']}&nb_page={$_GET['nb_page']}&supprimer=1'>Supprimer</a></div>
							</div>";
					}
					echo "</div>";
				}
		...
		<div id="pagination">
			<?php
				for ($i = 1; $i <= $nb_page; $i++)
				{
					...
					{
						echo "<a id='sous_pagination'href='indexView.php?page=gallery1&nb_page=$i'>" . $i . "</a>";
					}
				}
			?>
		</div>
